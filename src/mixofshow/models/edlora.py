...
class OrthogonalBasis:
    def __init__(self, size):
        self.size = size
        self.basis = self.generate_orthogonal_basis(size)

    def generate_orthogonal_basis(self, size):
        q, _ = torch.linalg.qr(torch.randn(size, size), mode='reduced')

        return q

    def get_random_subset(self, rank, name, concept_name):
        keys_string = ",".join(concept_name)
        keys_string = f'{name},{keys_string}'
        torch.manual_seed(hash(keys_string) % (2**32))
        indices = torch.randperm(self.size)[:rank]
        # print(f'Random indices for {keys_string}: {indices}')
        torch.manual_seed(0)
        return self.basis[:, indices]
    

orthogonal_basis_320 = OrthogonalBasis(320)
orthogonal_basis_640 = OrthogonalBasis(640)
orthogonal_basis_1280 = OrthogonalBasis(1280)


class LoRALinearLayer(nn.Module):
    def __init__(self, name, original_module, concept_name=None, rank=4, alpha=1, orthogonal = False):
        super().__init__()
        ...
        if (orthogonal):
            ...
            with torch.no_grad():
                if (out_features == 320):
                    self.lora_up.weight.copy_(orthogonal_basis_320.get_random_subset(rank, self.name, self.concept_name))
                elif (out_features == 640):
                    self.lora_up.weight.copy_(orthogonal_basis_640.get_random_subset(rank, self.name, self.concept_name))
                elif (out_features == 1280):
                    self.lora_up.weight.copy_(orthogonal_basis_1280.get_random_subset(rank, self.name, self.concept_name))
                # self.lora_up.weight.copy_(orthogonal_basis_320.get_random_subset(rank))

            self.lora_up.weight.requires_grad = False  # Fix the weights of lora_up
        else:
            torch.nn.init.kaiming_uniform_(self.lora_down.weight, a=math.sqrt(5))
