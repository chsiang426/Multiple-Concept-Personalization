...
class RegionT2I_AttnProcessor:
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None, region_list=None, height=None, width=None):
        ...
        if is_cross:
            ...
            for region in region_list:
                ...
                new_region_list.append((region_key, region_value, region[1]))

            hidden_states = self.region_rewrite(
                attn=attn,
                hidden_states=hidden_states,
                query=query,
                region_list=new_region_list,
                height=height,
                width=width)
    ...
    def _encode_region_prompt(self,
        ...
        if prompt_embeds is None:
            ...
            for idx, region in enumerate(region_list):
                ...
                # print(region_embeds.shape, region_neg_embeds.shape)
                region_list[idx] = (torch.cat([region_neg_embeds, region_embeds]), pos)
                # print(region_list[idx][0].shape)
