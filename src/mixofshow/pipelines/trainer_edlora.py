...
class EDLoRATrainer(nn.Module):
    def __init__(
        ...
        gradient_checkpoint=False,                  # mix-of-show, orthogonal, attn2-kv
        method='mix-of-show'
    ):
        super().__init__()
    ...
    def set_finetune_cfg(self, finetune_cfg):
        ...
        if finetune_cfg['unet']['enable_tuning'] and finetune_cfg['unet'].get('lora_cfg'):
            ...
            params_list = []
            concept_name = self.new_concept_cfg.keys()  # Get the concept name
            for name, module in self.unet.named_modules():
                if module.__class__.__name__ == where:
                    for child_name, child_module in module.named_modules():
                        if (self.method == 'mix-of-show'):
                            if child_module.__class__.__name__ == 'Linear' or (child_module.__class__.__name__ == 'Conv2d' and child_module.kernel_size == (1, 1)):
                                ...
                                params_list.extend(list(lora_module.parameters()))
                        elif (self.method == 'attn2-kv'):
                            if child_module.__class__.__name__ == 'Linear' and (child_name == 'to_k' or child_name == 'to_v') and (name.split('.')[-1] == 'attn2'):
                                lora_module = LoRALinearLayer(name + '.' + child_name, child_module, **unet_cfg['lora_cfg'])
                                self.unet_lora.append(lora_module)
                                params_list.extend(list(lora_module.parameters()))
                        elif (self.method == 'orthogonal'):
                            if child_module.__class__.__name__ == 'Linear' or (child_module.__class__.__name__ == 'Conv2d' and child_module.kernel_size == (1, 1)):
                                lora_module = LoRALinearLayer(name + '.' + child_name, child_module, concept_name=concept_name, **unet_cfg['lora_cfg'], orthogonal=True)
                                self.unet_lora.append(lora_module)
                                params_list.extend(list(lora_module.parameters()))
                        else:
