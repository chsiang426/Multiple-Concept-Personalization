...
def merge_kv_in_cross_attention(concept_list, new_concept_cfg, tokenizer, text_encoder, unet, unet_crosskv_list, device):
    ...
    for layer_idx, layer_name in cross_kv_layer_names:
        ...
        for concept, tuned_state_dict in zip(concept_list, unet_crosskv_list):
            ...
            if lora_up_name in tuned_state_dict:
                ...
                merged_params += alpha * lora_param

        new_kv_weights[layer_name] = merged_params
...
def merge_spatial_attention(concept_list, new_concept_cfg, tokenizer, text_encoder, unet, unet_spatial_attn_list, device):
    # spatial attention layer names
    matches = ['attn2.to_q', 'attn2.to_out.0', 'attn1.to_q', 'attn1.to_k', 'attn1.to_v', 'attn1.to_out.0', 'ff.net.2', 'ff.net.0.proj', 'proj_out', 'proj_in']

    spatial_attention_layer_names = []

    # the spatial attention name should match the order down->mid->up, and record its layer id
    for name, _ in unet.down_blocks.named_parameters():
        if any([x in name for x in matches]):
            spatial_attention_layer_names.append('down_blocks.' + name)

    for name, _ in unet.mid_block.named_parameters():
        if any([x in name for x in matches]):
            spatial_attention_layer_names.append('mid_block.' + name)

    for name, _ in unet.up_blocks.named_parameters():
        if any([x in name for x in matches]):
            spatial_attention_layer_names.append('up_blocks.' + name)

    logging.info(
        f'Unet have {len(spatial_attention_layer_names)} linear layer (related to spatial attention) need to optimize'
    )
    ...
    for layer_name in spatial_attention_layer_names:
        ...
        for concept, tuned_state_dict in zip(concept_list, unet_spatial_attn_list):
            ...
            lora_down_name = layer_name.replace('.weight', '.lora_down.weight')
            lora_up_name = lora_down_name.replace('lora_down', 'lora_up')
        ...
        new_spatial_attention_weights[layer_name] = merged_params
...
def compose_concepts(concept_cfg, optimize_textenc_iters, optimize_unet_iters, pretrained_model_path, save_path, suffix, device):
    ...
    if any([item is not None for item in unet_crosskv_list]):
        ...
        new_kv_weights = merge_kv_in_cross_attention(
            concept_list, new_concept_cfg, tokenizer, text_encoder, unet, unet_crosskv_list, device)
    ...
    if any([item is not None for item in unet_spatial_attn_list]):
        ...
        new_spatial_attention_weights = merge_spatial_attention(
            concept_list, new_concept_cfg, tokenizer, text_encoder, unet, unet_spatial_attn_list, device)
        unet_state_dict = unet.state_dict()
