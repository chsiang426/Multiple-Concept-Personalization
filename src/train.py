...
def train(root_path, args):
    ...
    EDLoRA_trainer, optimizer, train_dataloader = accelerator.prepare(EDLoRA_trainer, optimizer, train_dataloader)
    ...
    while global_step < opt['train']['total_iter']:
        ...
        if accelerator.sync_gradients:
            ...
            if global_step % opt['logger']['save_checkpoint_freq'] == 0:
                save_and_validation(accelerator, opt, EDLoRA_trainer, global_step, logger)

    # Save the lora layers, final eval
    accelerator.wait_for_everyone()
    save_and_validation(accelerator, opt, EDLoRA_trainer, 'latest', logger)


def save_and_validation(accelerator, opt, EDLoRA_trainer, global_step, logger):
