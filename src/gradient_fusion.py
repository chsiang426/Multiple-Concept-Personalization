...
TEMPLATE_SIMPLE_STYLE = 'photo in the {} style'
...
def parse_new_concepts(concept_cfg):
    ...
    # concept_label = [concept['label'] for concept in concept_list]
...
def merge_kv_in_cross_attention(concept_list, optimize_iters, new_concept_cfg,
    ...
    for concept, tuned_state_dict in zip(concept_list, unet_crosskv_list):
        if (concept['label'] == 'object'):
            ...
            concept_prompt = [
                TEMPLATE_SIMPLE_STYLE.format(concept['concept_name']),
                concept['concept_name']
            ]
        ...
        for layer_idx, layer_name in cross_kv_layer_names:
            ...
            # print(new_concept_input_dict)
            # print(new_concept_output_dict)
            # print(merge_params.shape, prompt_feature.shape)
            # torch.Size([320, 768]) torch.Size([6, 768])

            # zheng: 
            # prompt feature is the test prompt feature
            # merge_params is the original params + lora (for each concept)
            # new_concept_input_dict is the prompt feature
            # new_concept_output_dict is the original output for each concept
...
def merge_spatial_attention(concept_list, optimize_iters, new_concept_cfg, tokenizer, text_encoder, unet, unet_spatial_attn_list, test_scheduler, device):
    ...
    for concept, tuned_state_dict in zip(concept_list, unet_spatial_attn_list):
        ...
        if (concept['label'] == 'object'):
            concept_prompt = TEMPLATE_SIMPLE.format(concept['concept_name'])
        else:
            concept_prompt = TEMPLATE_SIMPLE_STYLE.format(concept['concept_name']) 
